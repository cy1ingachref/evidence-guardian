...
def fetch_url():
    ...
    try:
        ...
        if "169.254" in url or "127.0.0.1" in url or "localhost" in url:
            # Simulate internal service response (don't actually reach AWS metadata)
...
def search():
    ...
    # Vulnerability: directly reflects user input into HTML-like response
...
def login():
    ...
    # Intentionally vulnerable: string formatting instead of parameterized query
    conn = sqlite3.connect(DB_PATH)
    try:
...
def login_redirect():
    ...
    # Vulnerability: no validation of the redirect target
